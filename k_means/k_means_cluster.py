...
# getting the range of values (for feature scaling)
# featureValues = []
# for key in data_dict.keys():
#    try:
#        data_dict[key]['salary']
#    except KeyError:
#        print "error: key ", 'salary', " not present"
#    value = data_dict[key]['salary']
#    if value != "NaN":
#        featureValues.append(float(value))

#print('min', min(featureValues))
#print('max', max(featureValues))
...
model = KMeans(n_clusters=3, random_state=0).fit(finance_features)
...
try:
    Draw(pred, finance_features, poi, mark_poi=True,
         name="clusters.pdf", f1_name=feature_1, f2_name=feature_2, f3_name=feature_3)
except NameError:
